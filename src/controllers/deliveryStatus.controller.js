...
const DeliveryStatusController = {
	createDeliveryStatus: async (req, res) => {
		try {
			...
			console.log(
				`Creating delivery status for user with ID ${userId} and product with ID ${productId}.`,
			)
	...
	getDeliveryStatusById: async (req, res) => {
		try {
			...
			console.error(
				'Error fetching delivery status by ID:',
				error.message,
			)
			res.status(500).send(error.message)
		}
	...
	deleteDeliveryStatusById: async (req, res) => {
		try {
			...
			if (!deliveryStatus) {
				console.log(
					`Delivery status with ID ${id} not found for deletion.`,
				)
				res.status(404).json({
					msg: 'Delivery status not found.',
				})
			...
			console.error(
				'Error deleting delivery status by ID:',
				error.message,
			)
			res.status(500).send(error.message)
		}
	...
	updateDeliveryStatusById: async (req, res) => {
		try {
			...
			console.log(
				`Updating delivery status with ID ${id} for user with ID ${userId} and product with ID ${productId}.`,
			)
			...
			if (!deliveryStatus) {
				console.log(
					`Delivery status with ID ${id} not found for update.`,
				)
				res.status(404).json({
					msg: 'Delivery status not found.',
				})
			...
			console.error(
				'Error updating delivery status by ID:',
				error.message,
			)
			res.status(500).send(error.message)
		}
