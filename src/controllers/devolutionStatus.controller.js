...
const DevolutionStatusController = {
	createDevolutionStatus: async (req, res) => {
		try {
			...
			console.log(
				`Creating devolution status for user with ID ${userId} and product with ID ${productId}.`,
			)
	...
	getAllDevolutionStatus: async (req, res) => {
		try {
			...
			console.error(
				'Error fetching all devolution status:',
				error.message,
			)
			res.status(500).send(error.message)
		}
	...
	getDevolutionStatusById: async (req, res) => {
		try {
			...
			console.log(
				`Retrieved devolution status with ID ${id} successfully.`,
			)
			res.status(200).json({
				devolutionStatus,
				msg: 'Operation completed successfully!',
			...
			console.error(
				'Error fetching devolution status by ID:',
				error.message,
			)
			res.status(500).send(error.message)
		}
	...
	deleteDevolutionStatusById: async (req, res) => {
		try {
			...
			if (!devolutionStatus) {
				console.log(
					`Devolution status with ID ${id} not found for deletion.`,
				)
				res.status(404).json({
					msg: 'Devolution status not found.',
				})
			...
			console.error(
				'Error deleting devolution status by ID:',
				error.message,
			)
			res.status(500).send(error.message)
		}
	...
	updateDevolutionStatusById: async (req, res) => {
		try {
			...
			console.log(
				`Updating devolution status with ID ${id} for user with ID ${userId} and product with ID ${productId}.`,
			)
			...
			if (!devolutionStatus) {
				console.log(
					`Devolution status with ID ${id} not found for update.`,
				)
				res.status(404).json({
					msg: 'Devolution status not found.',
				})
			...
			console.error(
				'Error updating devolution status by ID:',
				error.message,
			)
			res.status(500).send(error.message)
		}
