...
const ShoppingCartController = {
  addToShoppingCart: async (req, res) => {
    try {
      ...
      console.log(`Adding items to shopping cart for user with ID ${userId}.`)
      ...
      for (const item of items) {
        ...
        if (!productExists) {
          console.log(`Product with ID ${productId} not found or insufficient stock.`)
          return res.status(400).json({
  ...
  deleteShoppingCartById: async (req, res) => {
    try {
      ...
      if (!shoppingCart) {
        console.log(`Shopping cart with ID ${id} not found for deletion.`)
        res.status(404).json({
          msg: 'Shopping cart not found.',
        })
        return
  ...
  updateShoppingCartById: async (req, res) => {
    try {
      ...
      if (!userExists) {
        ...
        console.log(`Product with ID ${existingProduct?.productId} not found.`)
        return res.status(400).json({
