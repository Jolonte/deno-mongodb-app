...
const UserController = {
	getAllUsers: async (req, res) => {
		try {
			...
			res.status(500).json({
				error: 'Internal Server Error: Unable to fetch all users',
			})
		}
	},
	...
	getUserById: async (req, res) => {
		try {
			...
			res.status(500).json({
				error: 'Internal Server Error: Unable to fetch user by id',
			})
		}
	},
	...
	deleteUserById: async (req, res) => {
		try {
			...
			res.status(500).json({
				error: 'Internal Server Error: Unable to delete user by id',
			})
		}
	},
	...
	updateUserById: async (req, res) => {
		try {
			...
			res.status(500).json({
				error: 'Internal Server Error: Unable to update user by id',
			})
		}
	},
}
