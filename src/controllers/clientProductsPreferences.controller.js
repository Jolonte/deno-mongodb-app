...
const ClientPreferencesController = {
  addToClientPreferences: async (req, res) => {
    try {
      ...
      console.log(`Adding to client preferences for user with ID ${userId}.`)
  ...
  getAllClientPreferences: async (req, res) => {
    try {
      ...
      console.error('Error fetching all client preferences:', error.message)
      res.status(500).send(error.message)
    }
  },
  ...
  getClientPreferencesById: async (req, res) => {
    try {
      ...
      console.log(`Retrieved client preference with ID ${id} successfully.`)
      res.status(200).json({
        clientPreference,
        msg: 'Operation completed successfully!',
      })
    } catch (error) {
      console.error('Error fetching client preference by ID:', error.message)
      res.status(500).send(error.message)
    }
  },
  ...
  deleteClientPreferencesById: async (req, res) => {
    try {
      ...
      if (!clientPreference) {
        console.log(`Client preference with ID ${id} not found for deletion.`)
        res.status(404).json({
          msg: 'Client preference not found.',
        })
        return
      ...
      console.error('Error deleting client preference by ID:', error.message)
      res.status(500).send(error.message)
    }
  },
  ...
  updateClientPreferencesById: async (req, res) => {
    try {
      ...
      console.log(`Updating client preference with ID ${id} for user with ID ${userId}.`)
      ...
      console.error('Error updating client preference by ID:', error.message)
      res.status(500).send(error.message)
    }
  },
}
